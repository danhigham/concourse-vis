  ...
  activate(state) {
    ...
    atom.workspace.observeActivePaneItem(function(item) {
      ...
      if (item.constructor.name == "TextEditor") {
        ...
        // add click for jobs and resrouces
        $('.node.job a').click(function(e) {
          var jobName = _.find(e.currentTarget.children, function(c) { return c.tagName == "text"; }).innerHTML;
          var buffer = _self.activeEditor.getBuffer();
          var lines = buffer.lines.slice(0);
        ...
        $('.node.input a, .node.output a, .node.constrained-input a').click(function(e) {
          var resourceName = _.find(e.currentTarget.children, function(c) { return c.tagName == "text"; }).innerHTML;
          var buffer = _self.activeEditor.getBuffer();
          var lines = buffer.lines.slice(0);
