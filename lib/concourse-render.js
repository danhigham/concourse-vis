...
graphLib = require('./graph.js')

module.exports.draw = function(svg, groups, jobs, resources, concourseVersion) {
...
module.exports.renderPipeline = function(groups) {
...
function createGraph(svg, groups, jobs, resources) {
  ...
  for (var i in jobs) {
    ...
    graph.setNode(id, new graphLib.Node({
      id: id,
      name: job.name,
      class: classes.join(" "),
      status: status,
  ...
  for (var i in jobs) {
    ...
    for (var j in job.outputs) {
      ...
      if (!jobOutputNode) {
        jobOutputNode = new graphLib.Node({
          id: outputId,
        ...
        for (var p in input.passed) {
          ...
          if (graph.node(sourceOutputNode)) {
            ...
            if (!graph.node(sourceInputNode)) {
              graph.setNode(sourceInputNode, new graphLib.Node({
                id: sourceInputNode,
                name: input.resource,
                key: input.resource,
    ...
    for (var j in job.inputs) {
      ...
      if (!input.passed || input.passed.length == 0) {
        ...
        if (!graph.node(inputId)) {
          ...
          graph.setNode(inputId, new graphLib.Node({
            id: inputId,
            name: input.resource,
            key: input.resource,
            class: classes,
