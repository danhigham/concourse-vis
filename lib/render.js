...
var d3 = require('d3');
var $ = require('jquery');
var graphLib = require('./graph.js')

var redraw;
module.exports.draw = function(svg, jobs, resources, newUrl) {
...
module.exports.createPipelineSvg = function(svg) {
...
module.exports.ResetPipelineFocus = resetPipelineFocus;
...
function createGraph(svg, jobs, resources) {
  ...
  for (var i in jobs) {
    ...
    graph.setNode(id, new graphLib.GraphNode({
      id: id,
      name: job.name,
      class: classes.join(" "),
      status: status,
  ...
  for (var i in jobs) {
    ...
    for (var j in job.outputs) {
      ...
      if (!jobOutputNode) {
        jobOutputNode = new graphLib.GraphNode({
          id: outputId,
        ...
        for (var p in input.passed) {
          ...
          if (graph.node(sourceOutputNode)) {
            ...
            if (!graph.node(sourceInputNode)) {
              graph.setNode(sourceInputNode, new graphLib.GraphNode({
                id: sourceInputNode,
                name: input.resource,
                key: input.resource,
    ...
    for (var j in job.inputs) {
      ...
      if (!input.passed || input.passed.length == 0) {
        ...
        if (!graph.node(inputId)) {
          graph.setNode(inputId, new graphLib.GraphNode({
            id: inputId,
            name: input.resource,
            key: input.resource,
